...
- **Infographic renderer** (`app/services/infographic_renderer.py`): AI image models (SD 1.5, FLUX) cannot spell — they generate garbled nonsense text in pixels. When a prompt contains keywords like `infographic`, `poster`, `chart`, `statistics`, `timeline`, `checklist`, the renderer: (1) generates structured text content via Cloudflare Workers AI LLM, (2) sanitizes the prompt to exclude text (`NO TEXT, NO WORDS, NO LETTERS`), (3) generates a text-free background via the normal image pipeline, (4) overlays correctly-spelled text via PIL with WorkSans fonts — uses numbered cyan badge circles (1, 2, 3...) instead of emoji icons because WorkSans doesn't support emoji glyphs. Wired into `/media/generate-image` and `/ai/generate-image`. Non-fatal: falls back to raw background if overlay fails. Vision-model QA rated output 9/10 with no spelling errors.

...
- **Every post must always have the correct media for its type** (single image/video, carousel, infographic/poster, Reel/TikTok, etc.). Never create, schedule, or publish with empty `media_ids`, mismatched asset types, missing carousel slides, or platform-rejected formats. Instagram Graph image URLs must be JPEG-compatible (`/api/v1/media/view?format=jpeg` for WebP/HEIC/AVIF). Fix or regenerate media before publish — do not ship broken media and retry later.
...
- **Platform focus (Visibility Era DAY 2, 8-week commitment)**: MAIN = LinkedIn (original content, carousels, Educator posts) → SECONDARY = Meta platforms (Instagram, Facebook Page, Threads — adapted versions) → LAST = Twitter/X + TikTok (opportunistic only). Scheduled workflows must target main/secondary tiers; never add schedules firing into last-tier platforms. Stored live as `brand_voices.voice_signature.platform_focus`; check with `creator_type.py platforms`.
...
- **Docker Model Runner (DMR)**: Primary local text inference. API at `http://localhost:12435` (host) or `http://host.docker.internal:12435` (containers) — an **engine-level GPU runner** (`docker-model-runner` container, image `local/model-runner:vllm-cuda-fixed`) with llama.cpp CUDA (`-ngl 999` full offload) + vLLM 0.27.1 backends. The Docker Desktop runner on 12434 (CPU-only llama.cpp) still exists but the app no longer points at it. OpenAI-compatible (`/engines/v1/chat/completions`), Anthropic-compatible (`/anthropic/v1/messages`), and Ollama-compatible (`/api/chat`) APIs. Models (platform-aware routing): `ai/qwen3:8b-q4_K_M` (`DMR_TEXT_MODEL` — long-form LinkedIn/Facebook + schema/JSON, ctx 6144, keep-alive 5m, ~5.1GB), `hf.co/unsloth/Qwen3-4B-Instruct-2507-GGUF:Q4_K_M` (`DMR_MID_MODEL`+`DMR_CHATBOT_MODEL` — non-thinking instruct for short-form platforms + all chatbots, ctx 4096, keep-alive 30m pinned, ~2.7GB), `ai/smollm3` (`DMR_TINY_MODEL` — <200-char prompts, `--reasoning-budget 0`), `ai/llama3.2` (carousel-pipeline NLP check/fix + title generation — required by `run_cloudless_carousel_pipeline`, ~2GB), `ai/qwen3-vl` (vision), `ai/qwen3-embedding` (embeddings), `ai/smollm2-vllm` (vLLM safetensors, experimental). Config via `docker model configure --context-size N`; active `docker model` context is `vllm` (→ localhost:12435). Skill: `.devin/skills/docker-model-runner/`. MCP server: `dmr` in `.devin/mcp_config.json` (8 tools: status, list, chat, embed, pull, inspect, configure, generate_image).
  - **DMR resilience**: `docker-model-runner` has `RestartPolicy=always`; models persist in named volume `docker-model-runner-models`. The `dmr-watchdog` compose service polls `/engines/v1/models` every 60s and `docker restart`s the runner after 3 failures (covers "container up, engine wedged"). Celery task `dmr_health.check_dmr_health` (every 5 min, default queue) publishes health + missing-model list to Redis `dmr:status`. `scripts/dmr/gpu-runner-recreate.sh` heals/recreates the container if Docker Desktop resets it.
  - **DMR image backup**: the patched image `local/model-runner:vllm-cuda-fixed` is exported to `/home/tbaltzakis/backups/dmr/model-runner-vllm-cuda-fixed.tar` (~9GB, torch cu126 fix + model configs baked in). If Docker Desktop wipes the image, restore with `docker load -i /home/tbaltzakis/backups/dmr/model-runner-vllm-cuda-fixed.tar` then run `scripts/dmr/gpu-runner-recreate.sh` — skips the install-runner + torch-patch + configure cycle. Re-export after committing a new image revision: `docker save local/model-runner:vllm-cuda-fixed -o /home/tbaltzakis/backups/dmr/model-runner-vllm-cuda-fixed.tar`.
  - **DMR settings**: `DMR_URL` (app, 12435 llama.cpp), `DMR_BASE_URL` (sidecars, no `/engines` suffix), `DMR_TEXT_MODEL`/`DMR_MID_MODEL`/`DMR_CHATBOT_MODEL`/`DMR_TINY_MODEL`/`DMR_VISION_MODEL`/`DMR_EMBEDDING_MODEL`, `DMR_MAX_CONCURRENCY` (default 4, semaphore in `dmr.py`), `DMR_VLLM_URL` (experimental vLLM backend — provider `dmr-vllm`, manual selection only, never in the auto fallback chain).
...
| DMR (primary local inference) | `docker-model-runner` (host engine, not Compose) | 12435 | `/engines/v1/models` | OK (200, model auto-loads on request) |
...
- **Always update the architecture documentation** whenever infrastructure, integrations, or system design changes — this is a standing requirement, not optional. Update `docs/application-architecture.md` (the canonical doc) plus any affected `AGENTS.md` / `README` notes in the same commit as the change.
...
- Health checks use worker-specific hostnames: `publishing@%h`, `media@%h`, `default@%h`, `messenger@%h`.
- Verify: `docker compose exec -T social-worker-publishing celery -A app.worker.celery_app inspect ping` — should show 4 nodes online.
...
- Other DMR models: `hf.co/unsloth/Qwen3-4B-Instruct-2507-GGUF:Q4_K_M` (`DMR_MID_MODEL`/`DMR_CHATBOT_MODEL` — short-form platforms + chatbots, non-thinking, pinned keep-alive 30m), `ai/qwen3-vl` (vision), `ai/qwen3-embedding` (embeddings), `ai/smollm3` (`DMR_TINY_MODEL`, `--reasoning-budget 0`). `ai/smollm2` remains pulled as rollback.
...
- **OMV-HA SSH access**: `ssh tbaltzakis@192.168.1.130` — password stored in `.env` as `OMV_SSH_PASSWORD` (never commit it). Runs dovecot (IMAP 143/993) + postfix (587, relays via Resend). OMV web UI on port 80. Reachable directly on LAN from this WSL2 host.

...
# Usage: ./personal-auto-reply.sh <account_id> [get|set] [enabled] [system_prompt]
set -euo pipefail
...
if [ "$ACTION" = "get" ]; then
  curl -s -H "Authorization: Bearer $TOKEN" \
    "http://localhost:8083/api/v1/messenger/${ACCOUNT_ID}/personal/auto-reply" | python3 -m json.tool
elif [ "$ACTION" = "set" ]; then
  ENABLED="${3:?Usage: $0 <account_id> set <true|false> [system_prompt]}"
  PROMPT="${4:-You are a helpful assistant. Reply concisely and professionally.}"

...
# Usage: sync-business.sh <account-id>
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/../../../.." && pwd)"
cd "$ROOT"

ID="${1:?Usage: sync-business.sh <account-id>}"
...
echo "Syncing business accounts for $ID..."
curl -sf -X POST "$API/api/v1/accounts/$ID/sync-business-accounts" \

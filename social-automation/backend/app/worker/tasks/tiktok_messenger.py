...
async def _process_account(
    ...
    bridge = BrowserBridgeClient(get_settings().BROWSER_BRIDGE_URL, platform='tiktok')
    replies_sent = 0
    account_name = account.display_name or account.username or "us"

    # 0. Check browser bridge session
    try:
        session = await bridge.ensure_session("tiktok")
        if session.get("status") != "active":
            logger.info(
                "TikTok DM: browser bridge session not active for account %s — %s",
    ...
    try:
        async with browser_session("tiktok", bridge) as b:
            convos_result = await b.get_tiktok_dm_conversations()
    except Exception as exc:
    ...
    for convo in conversations[:20]:
        ...
        try:
            ...
            async with browser_session("tiktok", bridge) as b:
                msgs_result = await b.get_tiktok_dm_messages(convo_id)
            ...
            async with browser_session("tiktok", bridge) as b:
                await b.send_tiktok_dm_message(convo_id, reply_text)

"""TikTok profile service — official API for reads, private + web API for writes.

The official TikTok Display API is read-only for profile fields.  To update
bio, nickname, username, or avatar we use two write strategies:

1. **Private mobile API** (``api-h2.tiktokv.com``) with a self-hosted
   X-Gorgon / X-Khronos request signer.  No third-party signing service is
   required.  **Status: blocked** — TikTok returns HTTP 403, likely due to
   changes in the signing algorithm or device fingerprint checks.

2. **Web API** (``/api/update/profile/``) via the authenticated browser
   session.  Requires ``X-Bogus`` header (generated by
   ``window.byted_acrawler.frontierSign()``), ``msToken`` cookie, and
   ``tt_csrf_token`` in the request body.  **Status: blocked** — TikTok's
   ``tt-ticket-guard-result: 1104`` anti-bot system silently rejects requests
   that lack a solved captcha verification token.  The slider captcha cannot
   be solved programmatically (synthetic mouse events lack ``isTrusted``).

**Current limitation**: TikTok profile writes (bio, nickname, username,
avatar) cannot be fully automated due to TikTok's multi-layer anti-bot
system (slider captcha + X-Bogus signing + tt-ticket-guard).  Profile reads
via the official Display API work correctly.  Profile writes require manual
intervention via the TikTok mobile app or website.

Auth for the private/web API uses a ``sessionid`` cookie extracted from a
logged-in TikTok web session.  The cookie is stored in the SecretStore under
...
Available profile operations:
    - get profile (official Display API)
    - set nickname (private API — currently blocked)
    - set signature/bio (private API — currently blocked)
    - set unique ID / username (private API — currently blocked)
    - check username availability (private API — currently blocked)

Web API endpoint (for browser-bridge implementation):
    POST /api/update/profile/?<query_params>
    Body: signature=<bio>&tt_csrf_token=<token>
    Headers: X-Bogus (from byted_acrawler.frontierSign)
    Limitation: tt-ticket-guard-result: 1104 (captcha required)
"""
...
class TikTokWriteBlocked(TikTokProfileError):
    """Raised when TikTok's anti-bot system blocks a profile write.

    TikTok requires a solved slider captcha for all profile writes (bio,
    nickname, username, avatar).  The captcha cannot be solved
    programmatically because:
    - Synthetic JavaScript mouse events lack the ``isTrusted`` property
    - The slider button is disabled until the captcha images fully load
    - The web API (``/api/update/profile/``) returns
      ``tt-ticket-guard-result: 1104`` (captcha required) even with valid
      X-Bogus signatures and msToken
    - The private mobile API (``api-h2.tiktokv.com``) returns HTTP 403

    Profile writes must be performed manually via the TikTok mobile app or
    website.
    """
    ...
    def __init__(self, detail: str = "") -> None:
        super().__init__(
            ...
            detail
            or (
                "TikTok profile writes are blocked by TikTok's anti-bot system "
                "(slider captcha + tt-ticket-guard). Please update the profile "
                "manually via the TikTok mobile app or website."
            ),
        )
    ...
    async def _post(
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if resp.status_code == 403:
                raise TikTokWriteBlocked(
                    f"Private API returned 403. TikTok's anti-bot system "
                    f"blocked the request. Path: {path}"
                )

...
Reverse-engineered from TikTok's client-side JavaScript (byted_acrawler /
frontierSign).  Based on the open-source implementation from
huuhuybn/X-Gorgon (MIT licensed).
...
Algorithm:
  ...
  4. Build a 19-byte payload: [0x40, ua_key(3), params_md5(2), body_md5(2),
     ua_md5(2), timestamp(4), magic(4), xor_checksum(1)]
  5. RC4-encrypt the payload with key [0xFF]
  6. Prepend [0x02, 0xFF]
  7. Custom Base64 encode with shifted alphabet

No browser needed.  No external TikTok libraries.  Pure stdlib.
"""

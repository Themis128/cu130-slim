...
class TestGetPlanLimits:
    def test_free_tier(self) -> None:
        ...
        assert limits["posts_per_month"] == 50
        assert limits["ai_calls_per_month"] == 100
        assert limits["social_accounts"] == 3

    def test_pro_tier(self) -> None:
        limits = get_plan_limits("pro")
        assert limits["posts_per_month"] == 500
        assert limits["ai_calls_per_month"] == 5000
        assert limits["social_accounts"] == 15

    def test_business_tier(self) -> None:
        limits = get_plan_limits("business")
        assert limits["posts_per_month"] == 2000
        assert limits["ai_calls_per_month"] == 20000
        assert limits["social_accounts"] == 50
...
class TestGetEffectiveLimit:
    def test_free_posts(self) -> None:
        assert get_effective_limit("free", "posts_per_month") == 50

    def test_free_ai_calls(self) -> None:
        assert get_effective_limit("free", "ai_calls_per_month") == 100

    def test_free_social_accounts(self) -> None:
        assert get_effective_limit("free", "social_accounts") == 3

    def test_business_posts(self) -> None:
        assert get_effective_limit("business", "posts_per_month") == 2000
    ...
    def test_unknown_tier_defaults_to_free(self) -> None:
        assert get_effective_limit("gold", "posts_per_month") == 50
        assert get_effective_limit("gold", "ai_calls_per_month") == 100

...
- Name: cloudflare-workers-ai.spec.ts >> Cloudflare backend contract @e2e >> catalog exposes Cloudflare Workers AI with frontend-aligned fields
- Location: tests/cloudflare-workers-ai.spec.ts:101:3
...
```ts
  ...
  23  | const TEST_EMAIL = `cf-e2e-${Date.now()}-${randomUUID().replace(/-/g, '').slice(0, 8)}@example.com`
